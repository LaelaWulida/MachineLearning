...
<xsl:template match="params">
	...
	<xsl:variable name="main.sFontCSS">color: <xsl:value-of select="$font-color-fixed" />; font-family: <xsl:value-of select="$main.sFontFamily"/>; font-size:<xsl:value-of select="font_size" />px;<xsl:choose><xsl:when test="font_style='bold'">font-weight: bold;</xsl:when><xsl:when test="font_style='italic'">font-style: italic;</xsl:when><xsl:when test="font_style='bolditalic'">font-weight: bold; font-style: italic;</xsl:when></xsl:choose><xsl:if test="text_shadow_strength!='none'">text-shadow: <xsl:value-of select="$main.sFontShadow"/>;</xsl:if></xsl:variable>
	...
	<xsl:variable name="main.sRadiusCSS"><xsl:if test="number(border_radius)!=0">border-radius: <xsl:value-of select="$main.sRadiusString"/>;</xsl:if></xsl:variable>
	...
	<xsl:variable name="style-box-shadow"><xsl:if test="$main.sShadowString!=''">-moz-box-shadow: <xsl:value-of select="$main.sShadowString"/>; -webkit-box-shadow: <xsl:value-of select="$main.sShadowString"/>; box-shadow: <xsl:value-of select="$main.sShadowString"/>;</xsl:if></xsl:variable>

	<xsl:variable name="css-cl-fld">width: <xsl:value-of select="number($width)-2*(number(border_width)+number(padding))"/>px; height: <xsl:value-of select="number($height)-2*(number(border_width)+number(padding))"/>px; padding: <xsl:value-of select="padding"/>px; text-align: <xsl:value-of select="text_align" />; border-width: <xsl:value-of select="border_width" />px; border-color: <xsl:value-of select="$border-color-fixed" />; border-style: <xsl:value-of select="border_style" />; background-color: <xsl:value-of select="$bg-color-fixed" />; <xsl:value-of select="$main.sFontCSS" /> -moz-border-radius: <xsl:value-of select="border_radius"/>px; -webkit-border-radius: <xsl:value-of select="border_radius"/>px; border-radius: <xsl:value-of select="border_radius"/>px; <xsl:value-of select="$style-box-shadow"/></xsl:variable>

	<div>
		<xsl:attribute name="class">clo-form_001_input</xsl:attribute>
		<div>
			...
			<xsl:choose>
				<xsl:when test="input_type='textarea'">
					<textarea>
						...
						<xsl:attribute name="style"><xsl:value-of select="$css-cl-fld"/></xsl:attribute>
						<xsl:attribute name="wrap"><xsl:value-of select="input_wrap" /></xsl:attribute>
				...
				<xsl:otherwise>
					<input>
						...
						<xsl:attribute name="class">cl-fld</xsl:attribute>
						<xsl:attribute name="style"><xsl:value-of select="$css-cl-fld"/></xsl:attribute>
						<xsl:if test="input_maxlength!=''"><xsl:attribute name="maxlength"><xsl:value-of select="input_maxlength" /></xsl:attribute></xsl:if>
